import numpy as np
import torch
import time
import argparse
from tqdm import tqdm
from model.network.imagediffusion import DiH_models, DiT_models
from torch.optim import AdamW
torch.set_float32_matmul_precision('high')
device = torch.device("cuda")

parser = argparse.ArgumentParser()
parser.add_argument("--batch-size", type=int, default=4)
parser.add_argument("--loop", type=int, default=100)
parser.add_argument("--model", type=str, default="XL/2")
parser.add_argument("--backward", type=bool, default=False)
args = parser.parse_args()

dims = np.array([16, 24, 32, 48, 64, 96, 128, 192, 256])
batch_size = args.batch_size
loop = args.loop
model_name = args.model


results = []

with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
    for idx, d in enumerate(dims):
        gen = torch.Generator()
        gen.manual_seed(3407)
        idx = idx+1
        b = batch_size//(idx**2)

        target = torch.randn((b, 4, d, d), generator=gen).to(device)

        model = DiH_models[f"DiH-{model_name}"](input_dim=4, n_classes=1000, im_size=d)
        for m in model.layers:
            m.compile()
        model = model.to(device)
        opt = AdamW(model.parameters(), lr=0.0001)

        # warmup
        print(f"Warmup d: {d}")
        for b in tqdm(range(10)):
            x = torch.randn((b, 4, d, d), generator=gen).to(device)
            c = torch.randint(0, 1000, (b,), generator=gen).to(device)
            t = torch.randint(0, 1000, (b,), generator=gen).to(device)

            y_pred = model(x, c, t)

            model.zero_grad()
            l2 = (target - y_pred).square().mean()
            l2.backward()
            opt.step()

        print(f"Test d: {d} (={d*16})")
        count = []
        for b in tqdm(range(loop)):
            x = torch.randn((b, 4, d, d)).to(device)
            c = torch.randint(0, 1000, (b,)).to(device)
            t = torch.randint(0, 1000, (b,)).to(device)

            start_time = time.perf_counter()
            y_pred = model(x, c, t)
            tmp = y_pred.cpu()
            if not args.backward:
                end_time = time.perf_counter()
                elapsed = end_time - start_time

            model.zero_grad()
            l2 = (target - y_pred).square().mean()
            l2.backward()
            opt.step()
            if args.backward:
                end_time = time.perf_counter()
                elapsed = end_time - start_time

            count.append(elapsed)
        elapsed = np.sum(count)
        print(f"d: {d}, tokens: {d*d}, elapsed: {elapsed} s, {elapsed/loop} s/batch, {elapsed/b/loop} s/image")
        print(f"tokens: {d*d}, tok/s: {d*d/(elapsed/b/loop)}")
        results.append(elapsed/b/loop)

    print(f"DiH images: {dims*16}")
    print(f"DiH tokens: {dims**2}")
    print(f"DiH s/image: {results}")
    for i in range(len(dims)):
        print(f"({dims[i]*16}, {results[i]})")
    for i in range(len(dims)):
        print(f"{dims[i]**2}, {dims[i]**2 / results[i]}")

    results = []

    for idx, d in enumerate(dims):
        gen = torch.Generator()
        gen.manual_seed(3407)
        idx = idx+1
        b = batch_size//(idx**2)

        target = torch.randn((b, 4, d, d), generator=gen).to(device)

        model = DiT_models[f"DiT-{model_name}"](input_dim=4, n_classes=1000, im_size=d)
        for m in model.layers:
            m.compile()
        model = model.to(device)
        opt = AdamW(model.parameters(), lr=0.0001)

        # warmup
        print(f"Warmup d: {d}")
        for b in tqdm(range(10)):
            x = torch.randn((b, 4, d, d), generator=gen).to(device)
            c = torch.randint(0, 1000, (b,), generator=gen).to(device)
            t = torch.randint(0, 1000, (b,), generator=gen).to(device)

            y_pred = model(x, c, t)

            model.zero_grad()
            l2 = (target - y_pred).square().mean()
            l2.backward()
            opt.step()


        print(f"Test d: {d} (={d*16})")
        count = []
        for b in tqdm(range(loop)):
            x = torch.randn((b, 4, d, d)).to(device)
            c = torch.randint(0, 1000, (b,)).to(device)
            t = torch.randint(0, 1000, (b,)).to(device)

            start_time = time.perf_counter()
            y_pred = model(x, c, t)
            tmp = y_pred[0].cpu()
            if not args.backward:
                end_time = time.perf_counter()
                elapsed = end_time - start_time

            model.zero_grad()
            l2 = (target - y_pred).square().mean()
            l2.backward()
            opt.step()
            if args.backward:
                end_time = time.perf_counter()
                elapsed = end_time - start_time

            count.append(elapsed)
        elapsed = np.sum(count)
        print(f"d: {d}, tokens: {d*d}, elapsed: {elapsed} s, {elapsed/loop} s/batch, {elapsed/b/loop} s/image")
        print(f"tokens: {d * d}, tok/s: {d * d / (elapsed / b / loop)}")
        results.append(elapsed/b/loop)

    print(f"DiT images: {dims*16}")
    print(f"DiT tokens: {dims**2}")
    print(f"DiT s/image: {results}")
    for i in range(len(dims)):
        print(f"({dims[i]*16}, {results[i]})")
    for i in range(len(dims)):
        print(f"{dims[i]**2}, {dims[i]**2 / results[i]}")

