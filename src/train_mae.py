...
def main(args):
    ...
    if args.precision == "bf16":
        args.precision_type = torch.bfloat16
    elif precision_type == "fp16":
        args.precision_type = torch.float16
    ...
    encoder = torch.nn.parallel.DistributedDataParallel(encoder.cuda(),
                                                      device_ids=[args.local_rank],
                                                      broadcast_buffers=False,
                                                      find_unused_parameters=True)
    decoder = torch.nn.parallel.DistributedDataParallel(decoder.cuda(),
                                                      device_ids=[args.local_rank],
                                                      broadcast_buffers=False,
                                                      find_unused_parameters=True)
    if args.load_checkpoint:
    ...
    for e in range(start_epoch, epoch):  # loop over the dataset multiple times
        with tqdm(train_ds, desc='Epoch={}'.format(e), disable=dist.get_rank() == 0) as tepoch:
            for imgs, lbls in tepoch:
                ...
                with torch.cuda.amp.autocast(dtype=args.precision_type, enabled=True):
                ...
                if dist.get_rank() == 0:
                    ...
                    if i % args.log_freq == 0:
                        ...
                        checkpoint = {"encoder": encoder.module.state_dict(),
