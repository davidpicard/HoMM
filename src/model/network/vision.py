...
class HoMVision(nn.Module):
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [
                HoMLayer(dim, order, order_expand, ffw_expand, dropout=dropout)
        ...
        nn.init.trunc_normal_(self.position, std=0.02, a=-2 * 0.02, b=2 * 0.02)
        self.cls = nn.Parameter(torch.randn((1, 1, dim)), requires_grad=True)
        nn.init.trunc_normal_(self.cls, std=0.02, a=-2 * 0.02, b=2 * 0.02)
    ...
    def forward(self, x, mask=None):
        ...
        if n == self.position.shape[1]:
            ...
            position = einops.rearrange(
                self.position, "b (m n) d ->b d m n", m=self.im_size // self.kernel_size
            )
