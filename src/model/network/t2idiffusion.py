...
class TextImageDiHBlock(nn.Module):
    def __init__(self, dim: int, order: int, order_expand: int, ffw_expand: int):
        ...
        self.gs = nn.Parameter(torch.zeros(1, 1, dim), requires_grad=True)
        self.gc = nn.Parameter(torch.zeros(1, 1, dim), requires_grad=True)
        self.gf = nn.Parameter(torch.zeros(1, 1, dim), requires_grad=True)
