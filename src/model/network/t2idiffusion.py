...
class TextImageDiHBlock(nn.Module):
    def __init__(self, dim: int, order: int, order_expand: int, ffw_expand: int):
        ...
        self.gs = nn.Parameter(-torch.ones(1, 1, dim), requires_grad=True)
        self.gc = nn.Parameter(-torch.ones(1, 1, dim), requires_grad=True)
        self.gf = nn.Parameter(-torch.ones(1, 1, dim), requires_grad=True)
    ...
    def __init__(self,
        ...
        def init_weights_(m):
            if isinstance(m, nn.Linear) or isinstance(m, nn.Conv2d):
                ...
                nn.init.normal_(m.weight, std=1/np.sqrt(fan_in + fan_out))
