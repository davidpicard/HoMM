...
class VideoHeunVelocitySampler():
    def __init__(self,
                 model):
        self.model = model
        self.train_timesteps = model.n_timesteps
    ...
    def sample(self, samples, txt, mask, temporal_mask=None, cfg: float = 0., num_inference_steps: int = 50, step_callback=None):
        ...
        for i, (t, tp1) in enumerate(zip(self.timesteps[0:-1], self.timesteps[1:])):
            # compute previous image: x_t -> x_t-1
            t = t * torch.ones((b, 1, 1, 1, 1)).to(samples.device)
            tp1 = tp1 * torch.ones((b, 1, 1, 1, 1)).to(samples.device)
            dt = (t - tp1)/self.train_timesteps

            di = self._predict(samples, t, txt, mask, temporal_mask, cfg)
            xi = samples - dt * di
...
class VideoFlowMatchingFrameByFrameSampler():
    def __init__(self,
                 model):
        self.model = model
        self.train_timesteps = model.n_timesteps
    ...
    def _predict(self, samples, t, txt, mask, cfg=0., neg_txt=None, neg_mask=None):
        if cfg > 0:
            b = samples.shape[0]
            if neg_txt is None:
            ...
            vid_input = torch.cat([samples, samples], dim=0)
            tim_input = torch.cat([t, t], dim=0).to(samples.device)
            txt_input = torch.cat([txt, neg_txt], dim=0).to(samples.device)
            mas_input = torch.cat([mask, neg_mask], dim=0).to(samples.device)
            # print(f"v: {vid_input.shape} t: {tim_input.shape} c: {txt_input.shape} m: {mas_input.shape}")
            pred = self.model.frame_by_frame_forward(vid_input, time=tim_input.squeeze(), txt=txt_input, mask=mas_input)
            eps_c = pred[0:b, ...]
            eps_u = pred[b:, ...]
            pred = eps_c + cfg * (eps_c - eps_u)
        else:
            pred = self.model.frame_by_frame_forward(samples, time=t.squeeze(), txt=txt, mask=mask)

        return pred

    @torch.no_grad()
    def sample(self, samples, txt, mask, temporal_mask=None, cfg: float = 0., neg_txt=None, neg_mask=None, num_inference_steps: int = 50, step_callback=None):
        b, n, c, h, w = samples.shape
        # set step values
        self.set_timesteps(num_inference_steps)
        for i, t in enumerate(self.timesteps):
            # compute previous image: x_t -> x_t-1
            t = t * torch.ones((b, 1, 1, 1, 1)).to(samples.device)
            samples = samples - 1./self.num_inference_steps*self._predict(samples, t, txt, mask,
                                                                          cfg, neg_txt=neg_txt, neg_mask=neg_mask)
            if step_callback is not None:
                step_callback(i, samples, samples)
        return samples
